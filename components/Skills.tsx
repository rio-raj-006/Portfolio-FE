        ...
        {skillEntries.length > 0 ? (
          skillEntries.map(([category, items]) => (
            <div key={category} className="animate-fade-in">
              <h3 className="text-xl font-bold text-emerald-400 mb-6 uppercase tracking-widest font-mono">
                // {category}
              </h3>
          ...
          <div className="text-center py-20 bg-zinc-900/30 rounded-2xl border border-zinc-800">
            <p className="text-zinc-500 italic">No skills found. Please ensure the backend API is delivering skill data correctly.</p>
          </div>
        )}
      </div>

      <div className="mt-20 p-8 rounded-2xl bg-gradient-to-br from-zinc-900 to-zinc-950 border border-zinc-800 text-center">
        <h4 className="text-zinc-400 mb-2 font-mono">Backend focused ecosystem</h4>
        <p className="text-zinc-500 italic">Continuously expanding horizons with modern architectures and data-driven solutions.</p>
      </div>
    </div>
  );
};
